...
\t                      <span class="th" id="updateAttachmentHelpTh">สูงสุด 10 ไฟล์ · ไฟล์ละไม่เกิน 20 MB · ระบบอัปโหลดไฟล์ก่อนส่งอีเมล</span>
    ...
    function loadDriveDatabaseFromCloud(options = {}) {
      ...
      return new Promise(resolve => {
        const script = document.createElement("script");
        let done = false;
        const finish = (success, allowLateResponse = false) => {
          if (done) return;
          done = true;
  ...
  <meta http-equiv="Expires" content="0">"""
    ...
    html = html.replace(
        '''                  <input type="hidden" name="owner" id="closeOwner">
                  <input type="hidden" name="closeDate" id="closeDate">''',
        '''                  <label class="form-field full">
                    <span class="bilingual-label"><span>Remark</span><span>หมายเหตุ</span></span>
                    <textarea class="input" name="remark" id="closeRemark" rows="3" placeholder="Enter closing remark / ระบุหมายเหตุการปิดเคส"></textarea>
                    <small class="form-hint">Optional closing note · หมายเหตุเพิ่มเติม (ไม่บังคับ)</small>
                  </label>
                  <label class="form-field full">
    ...
    new_close_handler = '''    document.querySelector("#closeCaseForm").addEventListener("submit", event => {
      ...
      addLogRecord({
        ...
        delayReason: remark,
        action: stage,
        actionReasonType: "Close Remark",
        actionReason: remark,
        actionReasonTypeTh: "หมายเหตุการปิดเคส",
        actionReasonTypeEn: "Close Remark",
      ...
      const emailBodyLines = [`Contract case ${contract.id} has been closed.`, `Status: ${outcomeEn}`, `Contract Name: ${contract.name}`, `Contract Owner: ${owner || "-"}`, `Close Date: ${formatDateForEmail(closeDate)}`];
      if (remark) emailBodyLines.push(`Remark: ${remark}`);
      emailBodyLines.push("", `เคสสัญญา ${contract.id} ปิดเรียบร้อยแล้ว`, `สถานะ: ${outcomeTh}`, `ชื่อสัญญา: ${contract.name}`, `เจ้าของสัญญา: ${owner || "-"}`, `วันที่ปิด: ${formatDateForEmail(closeDate)}`);
      if (remark) emailBodyLines.push(`หมายเหตุ: ${remark}`);
      const emailDraft = {
